...
### Issue: Lesson 404 Despite File Existing (Feb 2026) - FIXED
...
**Root Cause:** `validate-lesson.js` was NOT checking Zod schema errors.

**Fix:** Added `zod-schema-valid` as Rule #0 in `validate-lesson.js`. Now catches all schema errors automatically:

```bash
node scripts/validate-lesson.js content/de-fa/A1/module-02/A1-M02-L01.json
# Output: ✓ zod-schema-valid (or shows specific errors)
```

**Common Schema Errors (now caught by validate-lesson.js):**
