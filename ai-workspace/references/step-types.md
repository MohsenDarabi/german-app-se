...
| `audioId` | No | Custom audio ID (default: `{step.id}-audio`) |
| `maxRepeats` | No | Override max audio repeats |
| `showHints` | No | Override hint visibility |

**Audio Generation:**
- Audio is auto-generated by `generate-audio.js`
- Default audio ID: `{step.id}-audio` (e.g., `dictation-1-audio`)
- Uses `targetText` as the TTS source

**Evaluation:**
- Binary correct/wrong (no percentage scoring)
